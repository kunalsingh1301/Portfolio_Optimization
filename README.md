...
spark = SparkSession.builder \
    .appName("PostLoginFlow_StarterChannel") \
    .enableHiveSupport() \
    .getOrCreate()
...
def read_ivr(path):
    df = spark.read.option("header", True).csv(path)
    return df.filter(col("ONE_FA").isin(post_login_values)) \
...
# --------------------------------------------------
# STARTER CUSTOMERS (NEW LOGIC: first contact)
# --------------------------------------------------
w_user = Window.partitionBy("user_id").orderBy("channel")  # using channel order since no timestamp
starter_df = combined_df.withColumn("rn", row_number().over(w_user)) \
                        .filter(col("rn") == 1) \
                        .groupBy("channel") \
                        .agg(countDistinct("user_id").alias("starter_customers"))

# --------------------------------------------------
# FOLLOW-UP AND CASES (OLD LOGIC)
# --------------------------------------------------
user_contact_cnt = combined_df.groupBy("user_id").agg(count("*").alias("user_total_contacts"))
...
# --------------------------------------------------
# SECOND / THIRD CHANNEL (OLD LOGIC)
# --------------------------------------------------
user_channels = combined_df.groupBy("user_id").agg(collect_set("channel").alias("channels_used"))

df = combined_df.join(user_channels, "user_id")
df = df.withColumn("other_channels", expr("filter(channels_used, x -> x != channel)"))

second_df = df.filter(size(col("other_channels")) >= 1) \
              .withColumn("sec_channel", col("other_channels")[0]) \
              .groupBy("channel","sec_channel").count()

third_df = df.filter(size(col("other_channels")) >= 2) \
